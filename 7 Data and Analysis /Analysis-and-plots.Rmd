...
my.orchard(m, mod="1", alpha=0.5,data = all_data, whisker=0.025, group = "Article.ID", xlab = "Effect size")+
     annotate("text", size=6, y=0.66, x=1.2, label=paste("-0.070 [-0.102, -0.038]"))+
...
my.orchard(research_field1, mod = "Broad.research.field.reordered", group = "Article.ID", data = all_data, whisker=0.08, xlab = "Effect size",
    alpha = 0.5, transfm = "tanh",  cb = FALSE)+
     annotate("text", size=6, y=0.5, x=5.13, label=paste("-0.120 [-0.188, -0.053]"))+
     annotate("text", size=6, y=0.5, x=4.13, label=paste("-0.092 [-0.153, -0.032]"))+
     annotate("text", size=6, y=0.5, x=3.13, label=paste("-0.072 [-0.113, -0.031]"))+
     annotate("text", size=6, y=0.5, x=2.13, label=paste("-0.011 [-0.067, 0.045]"))+
     annotate("text", size=6, y=0.5, x=1.13, label=paste("0.006 [-0.053, 0.066]"))+
...
ggplot(all_data_long, aes(x=factor(Period), y=as.numeric(value), color=factor(Broad.research.field.reordered),  group = Broad.research.field.reordered)) + 
  geom_point(position = position_jitter(width = .0)) + 
  geom_smooth(method = 'lm', se = FALSE) + 
  labs(
...
my.orchard(authorship_position, mod = "Specific.productivity.measure.reordered", group = "Article.ID", data = measured_data, whisker = 0.08, xlab = "Effect size",
    alpha = 0.5)+
     annotate("text", size=6, y=-0.4, x=1.2, label=paste("-0.034 [-0.092, 0.023]"))+
     annotate("text", size=6, y=-0.4, x=2.2, label=paste("-0.046 [-0.178, 0.087]"))+
     annotate("text", size=6, y=-0.4, x=3.2, label=paste("-0.031 [-0.068, 0.006]"))+
     annotate("text", size=6, y=-0.4, x=4.2, label=paste("-0.016 [-0.111, 0.078]"))+
     annotate("text", size=6, y=-0.4, x=5.2, label=paste("-0.047 [-0.127, -0.034]"))+
  scale_fill_manual(values=c("#648FFF","#785EF0","#DC267F","#FE6100","#FFB000"))
